...
strDateiname = Application.GetSaveAsFilename(InitialFileName:=strVerzeichnis & knd & "_" & format & "_" & produkt & "_" & auflage & ".xls", FileFilter:="Microsoft Excel-Arbeitsmappe (*.xls), *.xls")
...
ActiveSheet.Cells((bytIndex + 27), (bytColumn * 2) - 1).Value = bytColorIndex
ActiveSheet.Cells((bytIndex + 27), bytColumn * 2).Interior.ColorIndex = bytColorIndex
Next bytIndex
Next bytColumn
End Sub
Sub Farbe_entfernen()
